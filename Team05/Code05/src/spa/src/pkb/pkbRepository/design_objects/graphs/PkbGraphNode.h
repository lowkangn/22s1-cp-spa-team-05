...
protected:
    ...
    explicit PkbGraphNode(shared_ptr<PkbEntity> entity) {
        this->entity = entity;
    }
...
public:
    ...
    */
    ...
    /*
        Equality via concurrent bfs or an option of shallow comparison.
        Note that the deep comparison assumes that the keys of each node are unique.
    */
    bool equals(shared_ptr<PkbGraphNode> other, bool shallow = false) {
        ...
        // NOTE: std== for unordered_set is not correct. see
        // https://stackoverflow.com/questions/13158682/c-stdset-equality
    ...
    /*
        Gets the keys of all connected nodes (by transitivity).
        i.e. the keys of all nodes traversable from this node will be included in this set.
    */
    ...
    */
    static shared_ptr<PkbGraphNode> createGraphFromAdjList(
        int rootNodeId, vector<vector<int>> adjList,
