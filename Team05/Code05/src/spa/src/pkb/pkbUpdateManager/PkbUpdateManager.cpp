...
shared_ptr<PkbEntity> PkbUpdateManager::externalEntityToPkbEntity(Entity entity) {
    if (entity.isVariableEntity()) {
        ...
        shared_ptr<PkbEntity> object = make_shared<PkbVariableEntity>(identifier);
        return object;
        ...
        if (entity.isPrintEntity()) {
            shared_ptr<PkbStatementEntity> statement =
                PkbStatementEntity::createPrintStatementEntity(lineNumber);
            ...
            shared_ptr<PkbStatementEntity> statement = PkbStatementEntity::createIfStatementEntity(
                lineNumber);
            shared_ptr<PkbEntity> object = shared_ptr<PkbEntity>(statement);
            return object;
        } else if (entity.isReadEntity()) {
            shared_ptr<PkbStatementEntity> statement =
                PkbStatementEntity::createReadStatementEntity(lineNumber);
            shared_ptr<PkbEntity> object = shared_ptr<PkbEntity>(statement);
            return object;
            ...
            shared_ptr<PkbStatementEntity> statement =
                PkbStatementEntity::createCallStatementEntity(lineNumber);
            shared_ptr<PkbEntity> object = shared_ptr<PkbEntity>(statement);
            return object;
        } else {
        ...
        // create pkb entity object
        string identifier = entity.getString();
        shared_ptr<PkbEntity> object = make_shared<PkbProcedureEntity>(identifier);
        return object;
    } else if (entity.isConstantEntity()) {
        int value = entity.getValue();
        shared_ptr<PkbEntity> object = make_shared<PkbConstantEntity>(value);
        return object;
    } else {
...
shared_ptr<PkbRelationship> PkbUpdateManager::externalRelationshipToPkbRelationship(
    ...
    if (relationship.isFollows()) {
        shared_ptr<PkbRelationship> pkbRelationship =
            make_shared<PkbFollowsRelationship>(lhsToPkbEntity, rhsToPkbEntity);
        return pkbRelationship;
    } else if (relationship.isFollowsStar()) {
        shared_ptr<PkbRelationship> pkbRelationship =
            make_shared<PkbFollowsStarRelationship>(lhsToPkbEntity, rhsToPkbEntity);
        return pkbRelationship;
    } else if (relationship.isModifies()) {
        shared_ptr<PkbRelationship> pkbRelationship =
            make_shared<PkbModifiesRelationship>(lhsToPkbEntity, rhsToPkbEntity);
        return pkbRelationship;
    } else if (relationship.isParent()) {
        shared_ptr<PkbRelationship> pkbRelationship =
            make_shared<PkbParentRelationship>(lhsToPkbEntity, rhsToPkbEntity);
        return pkbRelationship;
    } else if (relationship.isParentStar()) {
        shared_ptr<PkbRelationship> pkbRelationship =
            make_shared<PkbParentStarRelationship>(lhsToPkbEntity, rhsToPkbEntity);
        return pkbRelationship;
    } else if (relationship.isUses()) {
        shared_ptr<PkbRelationship> pkbRelationship =
            make_shared<PkbUsesRelationship>(lhsToPkbEntity, rhsToPkbEntity);
        return pkbRelationship;
    } else if (relationship.isCallsStmtAttribute()) {
        shared_ptr<PkbRelationship> pkbRelationship =
            make_shared<PkbCallStmtAttributeRelationship>(lhsToPkbEntity, rhsToPkbEntity);
        return pkbRelationship;
    } else if (relationship.isCalls()) {
        shared_ptr<PkbRelationship> pkbRelationship =
            make_shared<PkbCallsRelationship>(lhsToPkbEntity, rhsToPkbEntity);
        return pkbRelationship;
    } else if (relationship.isCallsStar()) {
        shared_ptr<PkbRelationship> pkbRelationship =
            make_shared<PkbCallsStarRelationship>(lhsToPkbEntity, rhsToPkbEntity);
        return pkbRelationship;
    } else if (relationship.isNext()) {
        shared_ptr<PkbRelationship> pkbRelationship =
            make_shared<PkbNextRelationship>(lhsToPkbEntity, rhsToPkbEntity);
        return pkbRelationship;
    } else {
...
vector<shared_ptr<PkbPattern>> PkbUpdateManager::externalPatternToPkbPattern(Pattern pattern) {
    ...
    if (pattern.isIfPattern()) {
        ...
        vector<string> variables = StringSplitter::splitStringByDelimiter(
            conditions,
            EXTERNAL_PATTERN_DELIMITER);

        // create if pattern for each variable and add it to table
        // we can do this now as we will only ever need to check if it's inside,
        // not an exact pattern
        ...
        vector<string> strings = {pattern.getLhs(), pattern.getRhs()};
...
void PkbUpdateManager::addRelationships(
...
void PkbUpdateManager::addPatterns(
    vector<Pattern> patterns, shared_ptr<PkbRepository> repository) {
...
void PkbUpdateManager::addCfgs(
    vector<shared_ptr<CFGNode>> rootNodes, shared_ptr<PkbRepository> repository) {
    /*
    ...
    */
    ...
    for (shared_ptr<CFGNode> rootNode : rootNodes) {
        ...
        while (!q.empty()) {
            ...
            shared_ptr<PkbStatementEntity> castedParent = static_pointer_cast<PkbStatementEntity>(
                this->externalEntityToPkbEntity(n->getEntity()));
            if (castedParent == NULL) {
                // conversion failed
                throw PkbException("Tried to convert cfg node to statement entity, but couldn't!");
            }
            shared_ptr<PkbGraphNode> parentNode =
                PkbControlFlowGraphNode::createPkbControlFlowGraphNode(castedParent);
            ...
            for (shared_ptr<CFGNode> child : n->getChildren()) {
                ...
                shared_ptr<PkbStatementEntity> castedChild = static_pointer_cast<
                    PkbStatementEntity>(this->externalEntityToPkbEntity(child->getEntity()));
                if (castedChild == NULL) {
                    // conversion failed
                    throw PkbException(
                        "Tried to convert cfg node to statement entity, but couldn't!");
                }
                shared_ptr<PkbGraphNode> childNode =
                    PkbControlFlowGraphNode::createPkbControlFlowGraphNode(castedChild);
                ...
                pair<string, string> edgeKey = pair<string, string>(
                    parentNode->getKey(),
                    childNode->getKey());
