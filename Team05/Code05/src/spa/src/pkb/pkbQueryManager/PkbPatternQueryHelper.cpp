...
vector<shared_ptr<PkbPattern>> PkbPatternQueryHelper::retrieveAssignPatterns(ClauseArgument lhs, ClauseArgument rhs, shared_ptr<PkbRepository> repository)
{
	// 1. parse lhs 
	...
	vector<shared_ptr<PkbPattern>> matchingPatterns = table->getAllThatMatchPostFixStrings(postFixStringsToMatch);

	return matchingPatterns;
}

vector<shared_ptr<PkbPattern>> PkbPatternQueryHelper::retrieveIfPatterns(ClauseArgument lhs, ClauseArgument rhs, shared_ptr<PkbRepository> repository)
{
	// parse lhs 
	// to be specific, we store only lhs patterns for if 
	// e.g. if has x, y, z -> we store x, y, z as individual patterns with lhs as their strings, rhs doesnt matter
	string lhsStringPattern;
...
vector<shared_ptr<PkbPattern>> PkbPatternQueryHelper::retrieveWhilePatterns(ClauseArgument lhs, ClauseArgument rhs, shared_ptr<PkbRepository> repository)
{
	// parse lhs 
	// to be specific, we store only lhs patterns for while
	// e.g. while has x, y, z -> we store x, y, z as individual patterns with lhs as their strings, rhs doesnt matter
	string lhsStringPattern;
