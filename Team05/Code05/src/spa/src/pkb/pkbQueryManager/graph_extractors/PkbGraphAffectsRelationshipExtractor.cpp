...
bool PkbGraphAffectsRelationshipExtractor::hasAffectsRelationship(
    shared_ptr<PkbControlFlowGraphNode> startNode,
    shared_ptr<PkbControlFlowGraphNode> endNode,
    ...
    */
    ...
    for (shared_ptr<PkbGraphNode> neighbour : neighbours) {
        // 2. cast
        shared_ptr<PkbControlFlowGraphNode> castedNeighbour = static_pointer_cast<
            PkbControlFlowGraphNode>(neighbour);
        ...
        if (castedNeighbour->getAsEntity() == endNode->getAsEntity()) {
            return true;
        } else if (!castedNeighbour->isWhileStatementNode()
                && !castedNeighbour->isIfStatementNode()) {
            // 3b. if not target, check and remove all candidates it modifies
            // early terminate if possible
            // search modifies
            ClauseArgument nodeAsLhs = ClauseArgument::createLineNumberArg(
                to_string(castedNeighbour->getStatementLineNumber()));
            ClauseArgument variable = ClauseArgument::createVariableArg("v");
            PkbRelationshipQueryHelper helper;
            vector<shared_ptr<PkbRelationship>> lhsModifies = helper.
            ...
            for (shared_ptr<PkbRelationship> relationship : lhsModifies) {
                ...
                if (candidateVariableKeyValueMap.find(key) != candidateVariableKeyValueMap.end()) {
                    erased.insert({key, candidateVariableKeyValueMap.at(key)});
                    // insert for back tracking
                    candidateVariableKeyValueMap.erase(key);
                }
                if (candidateVariableKeyValueMap.empty()) {
                    // backtrack
        ...
        if (castedNeighbour->getStatementLineNumber() <= startNode->getStatementLineNumber()) {
            // cycle
            // if been here for the second time, terminate
            if (this->cycleVisited.count(castedNeighbour->getKey())) {
                return false;
            } else {
                // else, mark, and continue
                this->cycleVisited.insert(castedNeighbour->getKey());
            }
        }

        // 5. recurse and return
        bool valueFromDownstream = this->hasAffectsRelationship(
            castedNeighbour,
            endNode,
            repository,
            candidateVariableKeyValueMap);
