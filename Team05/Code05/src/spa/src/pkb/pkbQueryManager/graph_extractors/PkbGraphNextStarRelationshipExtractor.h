...
private:
    ...
    // object-level variable as method is recursive
    ...
    */
    vector<shared_ptr<PkbControlFlowGraphNode>> extractAllFromStartDfs(
        shared_ptr<PkbControlFlowGraphNode> startNode);

    /*
        Cumulatively extracts all nextstar relationships that end at the specified node,
        starting from the specified start node.
    */
    vector<shared_ptr<PkbControlFlowGraphNode>> extractAllThatReachEndDfs(
        shared_ptr<PkbControlFlowGraphNode> startNode,
