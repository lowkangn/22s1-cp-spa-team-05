...
vector<shared_ptr<PkbControlFlowGraphNode>>
PkbGraphNextStarRelationshipExtractor::extractAllFromStartDfs(
    ...
    for (shared_ptr<PkbGraphNode> n : neighbours) {
        ...
        for (shared_ptr<PkbGraphNode> c : downstream) {
            ...
            shared_ptr<PkbRelationship> toAdd = make_shared<PkbNextStarRelationship>(
                startNode->getAsEntity(), c->getAsEntity());
            if (!this->extractedRelationshipKeys.count(toAdd->getKey())) {
                // not already found
...
vector<shared_ptr<PkbControlFlowGraphNode>>
PkbGraphNextStarRelationshipExtractor::extractAllThatReachEndDfs(
    ...
    for (shared_ptr<PkbGraphNode> neighbour : neighbours) {
        ...
        for (shared_ptr<PkbGraphNode> child : downstream) {
            // create relationship and add
            shared_ptr<PkbRelationship> toAdd = make_shared<PkbNextStarRelationship>(
                startNode->getAsEntity(), child->getAsEntity());
            if (!this->extractedRelationshipKeys.count(toAdd->getKey())) {
                // not already found
                this->extractedRelationships.push_back(toAdd);
                this->extractedRelationshipKeys.insert(toAdd->getKey());
