...
PQLPattern PkbQueryManager::pkbPatternToPqlPattern(shared_ptr<PkbPattern> pattern) {
    ...
    return PQLPattern::generateAssignPattern(
        pattern->getStatementLineNumber(),
        variableIdentifier);
}
...
*/
vector<PQLEntity> PkbQueryManager::retrieveAllProcedureEntities(
    shared_ptr<PkbRepository> repository) {
    // get
    vector<shared_ptr<PkbEntity>> all =
        this->entityHelper.retrieveAllProcedureEntities(repository);
...
*/
vector<PQLEntity> PkbQueryManager::retrieveAllStatementEntities(
    shared_ptr<PkbRepository> repository) {
    vector<PQLEntity> out;
    vector<shared_ptr<PkbEntity>> all =
        this->entityHelper.retrieveAllStatementEntities(repository);
...
*/
vector<PQLEntity> PkbQueryManager::retrieveStatementEntitiesByType(
    PKBTrackedStatementType pkbTrackedStatementType, shared_ptr<PkbRepository> repository) {
    // 0. get all statements
    vector<shared_ptr<PkbEntity>> statements = this->entityHelper.retrieveAllStatementEntities(
        repository);

    // 1. if need to filter
    PkbStatementEntityFilter filter = this->entityHelper.getFilterToCheckForStatementType(
        PkbStatementType::ASSIGN,
        true);
    if (pkbTrackedStatementType != PKBTrackedStatementType::ALL) {
        // convert type
        PkbStatementType converted = this->pkbTrackedStatmentTypeToInternalType(
            pkbTrackedStatementType);
...
*/
optional<PQLEntity> PkbQueryManager::retrieveStatementByLineNumberAndType(
    ...
    shared_ptr<PkbEntity> result = this->entityHelper.retrieveEntityByTypeAndKey(
        key,
...
*/
optional<PQLEntity> PkbQueryManager::retrieveProcedureEntityByName(
...
*/
optional<PQLEntity> PkbQueryManager::retrieveVariableByName(
    string name, shared_ptr<PkbRepository> repository) {
...
*/
optional<PQLEntity> PkbQueryManager::retrieveConstantByValue(
    int value, shared_ptr<PkbRepository> repository) {
    // create an object out of it and use it to get a key
...
vector<PQLRelationship> PkbQueryManager::retrieveRelationshipByTypeAndLhsRhs(
    PKBTrackedRelationshipType relationshipType, ClauseArgument lhs, ClauseArgument rhs,
    shared_ptr<PkbRepository> repository, bool optimized) {

    // 1. translate the type
    PkbRelationshipType convertedRelationshipType = this->pkbTrackedRelationshipTypeToInternalType(
        relationshipType);
    ...
    case PkbRelationshipType::AFFECTSSTAR:
        found = this->relationshipHelper.retrieveRelationshipsFromGraphsByTypeAndLhsRhs(
            convertedRelationshipType,
            lhs,
            rhs,
            repository,
            optimized);
    ...
    case PkbRelationshipType::USES: default:
        found = this->relationshipHelper.retrieveRelationshipsFromTablesByTypeAndLhsRhs(
            convertedRelationshipType,
            lhs,
            rhs,
            repository);
    ...
    for (shared_ptr<PkbRelationship> r : found) {
        out.push_back(
            PQLRelationship(
                this->pkbEntityToQpsPqlEntity(r->getLhs()),
                this->pkbEntityToQpsPqlEntity(r->getRhs())));
    }
    return out;
}


vector<PQLPattern> PkbQueryManager::retrievePatterns(
