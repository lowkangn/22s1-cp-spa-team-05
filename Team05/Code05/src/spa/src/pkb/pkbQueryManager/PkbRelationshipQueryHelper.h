...
private:
    ...
    */
    ...
    // ******************** helper methods ********************
    /*
        Helper function to check if retrieving the relationship,
        while semantically and syntactically correct, is even
        possible. e.g. Follows(s,s) is not possible
    */
    bool canShortCircuitRetrieveRelationshipByTypeAndLhsRhs(
        ...
        if (relationshipType == PkbRelationshipType::PARENT || relationshipType ==
            PkbRelationshipType::PARENTSTAR || relationshipType == PkbRelationshipType::FOLLOWS ||
            relationshipType == PkbRelationshipType::FOLLOWSSTAR || relationshipType ==
        ...
        } else if (relationshipType == PkbRelationshipType::CALLS || relationshipType ==
    ...
    */
    shared_ptr<PkbEntity> convertClauseArgumentToPkbEntity(ClauseArgument clause) {
        ...
        if (clause.isLineNumber()) {
            shared_ptr<PkbEntity> entity = PkbStatementEntity::createPrintStatementEntity(
                clause.getLineNumber());
            return entity;
        } else if (clause.isStringLiteral()) {
            shared_ptr<PkbEntity> entity = make_shared<PkbVariableEntity>(clause.getIdentifier());
            return entity;
        } else {
    ...
    */
    shared_ptr<PkbRelationship> findPkbRelationshipFromRepository(
        shared_ptr<PkbRelationship> relationship, shared_ptr<PkbRepository> repository) {
        // 1. key
        string key = relationship->getKey();

        // 2. query the table
        shared_ptr<PkbRelationship> found = repository->getRelationshipTableByRelationshipType(
            relationship->getType())->get(key);
        return found;
    }
    ...
    */
    shared_ptr<PkbEntity> findEntityFromRepository(
        shared_ptr<PkbEntity> entity, shared_ptr<PkbRepository> repository) {
        // 1. key
        string key = entity->getKey();

        // 2. query the table
        shared_ptr<PkbEntity> found = repository->getEntityTableByType(entity->getType())
            ->get(key);
        return found;
    }
    ...
    */
    void addPkbRelationship(
        shared_ptr<PkbRelationship> relationship, shared_ptr<PkbRepository> repository) {
        repository->getRelationshipTableByRelationshipType(relationship->getType())
            ->add(relationship);
    }
    ...
    */
    vector<shared_ptr<PkbRelationship>> retrieveNextStarByTypeAndLhsRhs(
    ...
    */
    vector<shared_ptr<PkbRelationship>> retrieveAffectsByTypeAndLhsRhs(
        ClauseArgument lhs, ClauseArgument rhs, shared_ptr<PkbRepository> repository,
        bool optimized);
    ...
    /*
        Handles the (complicated) logic of retrieval for affects* relationships.
    */
    vector<shared_ptr<PkbRelationship>> retrieveAffectsStarByTypeAndLhsRhs(
        ClauseArgument lhs, ClauseArgument rhs, shared_ptr<PkbRepository> repository,
        bool optimized);
    ...
    */
    unordered_map<string, shared_ptr<PkbEntity>> getLhsModifiesRhsUsesIntersectingVariableMap(
        ClauseArgument lhs, ClauseArgument rhs, shared_ptr<PkbRepository> repository) {
        // 2.1 find all v lhs modifies and convert to set
        ClauseArgument variable = ClauseArgument::createVariableArg("v");
        vector<shared_ptr<PkbRelationship>> lhsModifies = this->
            retrieveRelationshipsFromTablesByTypeAndLhsRhs(
        ...
        vector<shared_ptr<PkbRelationship>> rhsUses = this->
            retrieveRelationshipsFromTablesByTypeAndLhsRhs(
...
public:
    ...
    */
    vector<shared_ptr<PkbRelationship>> retrieveRelationshipsFromTablesByTypeAndLhsRhs(
        PkbRelationshipType relationshipType, ClauseArgument lhs, ClauseArgument rhs,
    ...
    */
    vector<shared_ptr<PkbRelationship>> retrieveRelationshipsFromGraphsByTypeAndLhsRhs(
        PkbRelationshipType relationshipType, ClauseArgument lhs, ClauseArgument rhs,
