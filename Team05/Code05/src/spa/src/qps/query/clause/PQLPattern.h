...
private:
    ...
    PQLPattern(PQLEntity& statementEntity, PQLEntity& variableEntity) : statementEntity(
                                                                            statementEntity),
                                                                        variableEntity(
                                                                            variableEntity) {
        ...
        if (!statementEntity.isStatement() || !variableEntity.isVariable()) {
            throw PQLLogicError(
                "Trying to create PQLPattern, but LHS is not statement, or RHS is not variable!");
        }
    }
...
public:
    ...
    */
    ...
    friend bool operator==(PQLPattern first, PQLPattern second) {
        return first.statementEntity == second.statementEntity && first.variableEntity == second.
            variableEntity;
    }
