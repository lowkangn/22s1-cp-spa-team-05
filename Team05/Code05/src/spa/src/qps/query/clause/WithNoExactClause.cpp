...
shared_ptr<ClauseResult> WithNoExactClause::execute(shared_ptr<PKBQueryHandler> pkb) {
    ...
    } else if (this->lhsAttribute.isStmtNumAttribute() && this->rhsAttribute.
                                                                isStmtNumAttribute()) {
        return this->handleBothSidesStmtNumAttribute(pkb);
    }

    // Evaluate each side of the with clause
    shared_ptr<ClauseResult> lhsResult = this->getSingleAttrRefResult(
        pkb,
        this->lhsSynonym,
        this->lhsAttribute);
    shared_ptr<ClauseResult> rhsResult = this->getSingleAttrRefResult(
        pkb,
        this->rhsSynonym,
        this->rhsAttribute);
...
shared_ptr<EntityClauseResult> WithNoExactClause::handleBothSidesEqual() {
    //both sides are exactly equal, so this with clause does not
    //add any constraint to the final result
    return make_shared<EntityClauseResult>(EntityClauseResult::createNonEmptyNoSynonymResult());
}

shared_ptr<ClauseResult> WithNoExactClause::handleBothSidesStmtNumAttribute(
    ...
    // == case should be handled by handleBothSidesEqual
    ...
    // If both sides are the same type, we only need to retrieve either side.
    // If one side is a stmt synonym, it suffices to retrieve the other side.
    ...
    ClauseResult finalResult = synonymToGetResult.mergeByForceInnerJoin(
        otherSynonymResult,
        synonymToGet,
        otherSynonym);
    return make_shared<ClauseResult>(finalResult);
...
ClauseArgument WithNoExactClause::getEqualityCheckColumn(
    ClauseArgument& synonym, ClauseArgument& attribute) {
    if (synonym.isProcedureSynonym() || synonym.isConstantSynonym() || synonym.
        isVariableSynonym()) {
        return synonym;
...
bool WithNoExactClause::equals(shared_ptr<WithClause> other) {
    ...
    shared_ptr<WithNoExactClause> otherWithRelationship =
        dynamic_pointer_cast<WithNoExactClause>(other);
