...
vector<ClauseArgument> ClauseResult::findConnectingArgs(const ClauseResult& otherResult) {
    ...
    for (ClauseArgument arg : otherResult.args) {
        if (!arg.isWildcard()
            && this->argumentToIndexMap.find(arg) != this->argumentToIndexMap.end()) {
...
vector<int> ClauseResult::getColumnIndices(const vector<ClauseArgument>& searchArgs) {
    ...
    for (const ClauseArgument& arg : searchArgs) {
        unordered_multimap<ClauseArgument, int>::iterator mapIter = this->argumentToIndexMap.find(
            arg);
...
ClauseResult ClauseResult::performInnerJoin(
    ClauseResult otherResult, const vector<ClauseArgument>& connectingArgs) {
    ...
    unordered_set<int> otherTableKeyColumnIndicesSet;
    // needed later for checking which elements to append
    ...
    for (Row otherTableRow : otherResult.table) {
        ...
        pair<multimap<Key, Row>::iterator, multimap<Key, Row>::iterator> range =
            tableMultimap.equal_range(key);
        for (multimap<Key, Row>::iterator mapIter = range.first;
            mapIter != range.second;
            ++mapIter) {
            // For each entry that with matching keys, append elements that aren't
            // already in the entry and add new row to the new table
...
void ClauseResult::duplicateExistingColumnAs(
    const ClauseArgument& headerOfColumnToDuplicate, const ClauseArgument& newColumnHeader) {
    vector<int> indices = this->getColumnIndices(
        vector<ClauseArgument>{headerOfColumnToDuplicate});
...
ClauseResult ClauseResult::mergeResult(const ClauseResult& resultToMerge) {
    ...
    // If there are connecting args, find common synonym columns
    // and inner join on those, otherwise perform cross product
...
ClauseResult ClauseResult::mergeByForceInnerJoin(
    ClauseResult resultToMerge, const ClauseArgument& leftOn, const ClauseArgument& rightOn) {
    bool leftHasArg = *find(this->args.begin(), this->args.end(), leftOn) == leftOn;
    bool rightHasArg = *find(
        resultToMerge.args.begin(),
        resultToMerge.args.end(),
        rightOn) == rightOn;
    ...
    for (const ClauseArgument& arg : connectingArgs) {
        if (arg != leftOn) {
            this->renameColumns(
                arg,
                ClauseArgument::createStmtArg(to_string(i) + leftPlaceholderSuffix));
        }
        if (arg != rightOn) {
            resultToMerge.renameColumns(
                arg,
                ClauseArgument::createStmtArg(to_string(i) + rightPlaceholderSuffix));
        }
        i++;
    }
    // perform join
...
ClauseResult ClauseResult::rearrangeTableToMatchSelectResults(
    const vector<ClauseResult>& selectResults) {
    ...
    for (const ClauseResult& currentResult : selectResults) {
        ...
        if (currentDesiredSynonymIndices.size() == 1) {
            ...
            if (currentDesiredSynonymIndices[0] != -1 && currentDesiredSynonymIndices[1] != -1) {
                // in table, return index to attribute column
                ...
                if (currentDesiredSynonymIndices[0] == -1) {
                    ...
                    ClauseResult synonymColumn = this->getColumnFromTable(
                        currentDesiredSynonymIndices[0]);
                    ClauseResult attributeColumn = synonymColumn.
                        convertSynonymsColumnToAttributesColumn(currentResult);
...
unordered_set<PQLEntity> ClauseResult::getEntitySet(const ClauseArgument& arg) {
    ...
    if (this->argumentToIndexMap.find(arg) == this->argumentToIndexMap.end()) {
        return column;
    }
