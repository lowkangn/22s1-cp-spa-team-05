...
shared_ptr<RelationshipClauseResult> CfgRelationshipClause::executeWithRestriction(shared_ptr<PKBQueryHandler> pkb,
...
shared_ptr<RelationshipClauseResult> CfgRelationshipClause::executeWithRestrictionHelper(shared_ptr<PKBQueryHandler>& pkb,
    unordered_map<ClauseArgument, unordered_set<PQLEntity>>& restrictionMap) {
    // Apply restrictions
...
shared_ptr<RelationshipClauseResult> CfgRelationshipClause::execute(shared_ptr<PKBQueryHandler> pkb) {
    //Query pkb
...
bool CfgRelationshipClause::hasRestriction(unordered_map<ClauseArgument, unordered_set<PQLEntity>>& restrictionMap, ClauseArgument& arg) {
    return (arg.isStmtSynonym() || arg.isAssignSynonym()) && restrictionMap.find(arg) != restrictionMap.end();
}

void CfgRelationshipClause::replaceSynonymByExactArg(vector<ClauseArgument>& oneSideArgs, ClauseArgument& originalSynonym,
        unordered_map<ClauseArgument, unordered_set<PQLEntity>>& restrictionMap) {
    ...
    unordered_map<ClauseArgument, unordered_set<PQLEntity>>::const_iterator mapIter = restrictionMap.find(originalSynonym);
...
bool CfgRelationshipClause::isWorthUpdating(
    ...
    int maxNumRelationships = restrictionMap.at(this->lhs).size() * restrictionMap.at(this->rhs).size();
