...
shared_ptr<ClauseResult> AttributeClause::getSingleAttrRefResult(
    ...
    if (synonym.isProcedureSynonym()) {
        ...
        return make_shared<EntityClauseResult>(
            synonym,
            pkb->retrieveStatementEntitiesByType(statementType));
    } else {
        ...
        if (synonym.isCallSynonym() && attribute.isProcNameAttribute()) {
            ...
            throw PQLLogicError("Unknown attrRef being executed");
        }
        ClauseArgument wildcardArg = ClauseArgument::createWildcardArg();
        return make_shared<RelationshipClauseResult>(
            synonym,
            attribute,
            pkb->retrieveRelationshipByTypeAndLhsRhs(
                relationshipToRetrieve,
                synonym,
                wildcardArg));
    }
}
