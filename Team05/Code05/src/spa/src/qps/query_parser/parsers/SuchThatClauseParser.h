...
public:
    ...
    /* Checks that the arguments fulfill the constraints of the clause;
     * throws a PQL error otherwise */
    virtual void checkArguments(list<ClauseArgument>& args) = 0;

    /* Creates the clause given the clause type and arguments */
    virtual shared_ptr<RelationshipClause> createClause(
        PQLToken clauseTypeToken, list<ClauseArgument>& args) = 0;
};
