    ...
    for (ClauseArgument arg : args) {
        if (arg.isStringLiteral()) {
            throw PQLSyntaxError(
                "Both args for Affects/Affects* must be statements, which cannot be a string");
        }
...
shared_ptr<RelationshipClause> AffectsParser::createClause(PQLToken clauseTypeToken,
    list<ClauseArgument>& args) {
    //Affects clause should have exactly 2 arguments
    assert(args.size() == 2);

    if (this->isStar) {
        return make_shared<AffectsTClause>(args.front(), args.back());
    } else {
        return make_shared<AffectsClause>(args.front(), args.back());
    }
}
