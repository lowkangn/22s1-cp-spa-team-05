...
void CallsParser::checkArguments(list<ClauseArgument>& args) {
	...
	for (ClauseArgument arg : args) {
		...
		if (!arg.isWildcard() && !arg.isProcedureSynonym() && !arg.isStringLiteral()) {
			this->semanticErrorMessage = "Each arg for Calls/Calls* must be must be a wildcard, a \"NAME\" or a declared procedure synonym";
		}
	}
}

shared_ptr<RelationshipClause> CallsParser::createClause(PQLToken clauseTypeToken, list<ClauseArgument>& args) {
	//Calls clause should have exactly 2 arguments
	assert(args.size() == 2);

	if (this->isStar) {
		return shared_ptr<RelationshipClause>(new CallsTClause(args.front(), args.back()));
	}
	else {
		return shared_ptr<RelationshipClause>(new CallsClause(args.front(), args.back()));
	}
}
