    ...
    for (ClauseArgument arg : args) {
        if (arg.isStringLiteral()) {
            throw PQLSyntaxError(
                "Both args for Next/Next* must be stmtRefs, which cannot be a string");
        }
...
shared_ptr<RelationshipClause> NextParser::createClause(PQLToken clauseTypeToken,
    list<ClauseArgument>& args) {
    //Next clause should have exactly 2 arguments
    assert(args.size() == 2);

    if (this->isStar) {
        return make_shared<NextTClause>(args.front(), args.back());
    } else {
        return make_shared<NextClause>(args.front(), args.back());
    }
}
