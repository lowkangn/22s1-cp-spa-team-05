...
vector<ClauseArgument> WithParser::parseRef() {
    ...
    if (!refOrSynonym.isSynonym() && !refOrSynonym.isLineNumber() && !refOrSynonym.
        isStringLiteral()) {
        throw PQLSyntaxError(
            "ref of a with clause should be a '\"'IDENT'\"', INTEGER or an attrRef");
    }
...
void WithParser::checkAttrCompare(
    vector<ClauseArgument>& lhsArgs, vector<ClauseArgument>& rhsArgs) {
    ...
    for (vector<ClauseArgument> refArgs : {lhsArgs, rhsArgs}) {
        if (refArgs.size() == 1) {
            ...
            areRefsIntegers.push_back(
                refArgs.back().isValueAttribute() || refArgs.back().isStmtNumAttribute());
        }
    }
    ...
    if (areRefsIntegers.front() != areRefsIntegers.back()) {
        this->semanticErrorMessage =
            "The two refs in an attrCompare must be of the same type (both NAME, or both INTEGER)";
    }
}
...
shared_ptr<WithClause> WithParser::createWithClause(
    vector<ClauseArgument>& lhsArgs, vector<ClauseArgument>& rhsArgs) {
    ...
    assert(exactArg.isExactReference() && nonExactArgs.front().isSynonym()
        && nonExactArgs.back().isAttributeName());
