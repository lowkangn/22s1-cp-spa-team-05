...
void SuchThatState::parseOneClause(
    ...
    if (token.isModifies()) {
        ...
        throw PQLSyntaxError(
            "Only Modifies, Uses, Parent/Parent*, Follows/Follows*, Calls/Calls*, Next/Next*, Affects/Affects* are supported as such that clauses."); //NOLINT(whitespace/line_length)
    }
