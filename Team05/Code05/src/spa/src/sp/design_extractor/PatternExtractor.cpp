    ...
    default: {
        if (!ast->isTerminal()) {
            ...
            for (shared_ptr<ASTNode> child : children) {
                ...
                patterns.insert(
                    patterns.end(),
                    extractedPatterns.begin(),
                    extractedPatterns.end());
            }
        }
    }
    }
...
vector<Pattern> PatternExtractor::handleIf(shared_ptr<ASTNode> ast) {
    ...
    extractedPatterns.insert(
        extractedPatterns.end(),
        elseExtractedPatterns.begin(),
        elseExtractedPatterns.end());
...
string PatternExtractor::recursiveExtractExpression(shared_ptr<ASTNode> ast) {
    ...
    if (ast->isTerminal()) {
        // node is terminal (Either VariableASTNode or ConstantValueASTNode)
        // can return the token as a string
        return ast->getToken().getString();
    } else if (numChildren == 1) {
        // Must be a bracketed expression (i.e. bracket node)
        ...
        shared_ptr<ExpressionASTNode> expressionNode =
            dynamic_pointer_cast<ExpressionASTNode>(ast);
        ...
        return recursiveExtractExpression(expressionNode->getLeftHandSide()) + DELIMITER +
            operatorString + DELIMITER +
            recursiveExtractExpression(expressionNode->getRightHandSide());
    } else {
