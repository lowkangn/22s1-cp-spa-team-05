...
vector<Relationship> UsesExtractor::handleProcedure(shared_ptr<ASTNode> ast) {
    ...
    if (procedureNameToRelationshipMap.find(procedureName) != procedureNameToRelationshipMap.
        end()) {
    ...
    for (shared_ptr<ASTNode> child : childContainer->getChildren()) {
        vector<Relationship> extractedRelationships = this->recursiveContainerExtract(
            leftHandSide,
            child);
...
vector<Relationship> UsesExtractor::handlePrint(shared_ptr<ASTNode> ast) {
    ...
    return vector<Relationship>{
        Relationship::createUsesRelationship(printEntity, printedVariable)
    };
}
...
vector<Relationship> UsesExtractor::handleWhile(shared_ptr<ASTNode> ast) {
    ...
    for (shared_ptr<ASTNode> child : childStmtLst->getChildren()) {
        vector<Relationship> extractedRelationships = this->recursiveContainerExtract(
            leftHandSide,
            child);
        usesRelationships.insert(
            usesRelationships.end(),
            extractedRelationships.begin(),
            extractedRelationships.end());
    }
...
vector<Relationship> UsesExtractor::handleIf(shared_ptr<ASTNode> ast) {
    ...
    for (shared_ptr<ASTNode> child : childrenInThen) {
        vector<Relationship> extractedThenRelationships = this->recursiveContainerExtract(
            leftHandSide,
            child);
        usesRelationships.insert(
            usesRelationships.end(),
    ...
    for (shared_ptr<ASTNode> child : childrenInElse) {
        vector<Relationship> extractedElseRelationships = this->recursiveContainerExtract(
            leftHandSide,
            child);
        usesRelationships.insert(
            usesRelationships.end(),
...
vector<Relationship> UsesExtractor::handleCall(shared_ptr<ASTNode> ast) {
    ...
    if (procedureNameToRelationshipMap.find(procedureCalledName) != procedureNameToRelationshipMap.
        ...
        vector<Relationship> convertedRelationships = extractCallRelationshipFromProcedure(
            ast,
...
vector<Entity> UsesExtractor::extractVariables(shared_ptr<ASTNode> ast) {
    ...
    if (ast->isVariable()) {
        ...
        shared_ptr<ExpressionASTNode> expressionNode = dynamic_pointer_cast<
            ExpressionASTNode>(ast);
...
void UsesExtractor::addRelationshipsWithVariables(
    vector<Relationship>& relationships, Entity& leftHandSide, vector<Entity>& variables) {
...
vector<Relationship> UsesExtractor::recursiveContainerExtract(
