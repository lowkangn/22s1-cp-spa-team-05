        ...
        if (ast->isProgramNode()) {
            for (int i = 0; i < children.size(); i++) {
                ...
                /* 
                    ...
                    extract on demand
                */
                this->allProcedures.emplace(child->extractEntity().getString(), child);
...
vector<Relationship> ModifiesExtractor::handleAssign(shared_ptr<ASTNode> ast) {
    ...
    modifiesRelationships.push_back(
        Relationship::createModifiesRelationship(leftHandSide, modifiedEntity));
...
vector<Relationship> ModifiesExtractor::handleRead(shared_ptr<ASTNode> ast) {
    ...
    modifiesRelationships.push_back(
        Relationship::createModifiesRelationship(leftHandSide, rightHandSide));
...
vector<Relationship> ModifiesExtractor::handleProcedure(shared_ptr<ASTNode> ast) {
    ...
    if (procedureNameToRelationshipMap.find(procedureName)
        != procedureNameToRelationshipMap.end()) {
    ...
    for (shared_ptr<ASTNode> child : childContainer->getChildren()) {
        vector<Relationship> extractedRelationships = recursiveContainerExtract(
            leftHandSide,
            child);
        extractedChildRelationships.insert(
            extractedChildRelationships.end(),
            extractedRelationships.begin(),
            extractedRelationships.end());
    }

    // Update procedureNameToRelationshipMapp for DP so future calls can refer to the result
...
vector<Relationship> ModifiesExtractor::handleIf(shared_ptr<ASTNode> ast) {
    ...
    for (shared_ptr<ASTNode> child : thenChild->getChildren()) {
        vector<Relationship> extractedThenRelationships = recursiveContainerExtract(
            leftHandSide,
            child);
        extractedChildRelationships.insert(
            extractedChildRelationships.end(),
            extractedThenRelationships.begin(),
            extractedThenRelationships.end());
    }

    for (shared_ptr<ASTNode> child : elseChild->getChildren()) {
        vector<Relationship> extractedElseRelationships = recursiveContainerExtract(
            leftHandSide,
            child);
        extractedChildRelationships.insert(
            extractedChildRelationships.end(),
            extractedElseRelationships.begin(),
            extractedElseRelationships.end());
    }
...
vector<Relationship> ModifiesExtractor::handleCall(shared_ptr<ASTNode> ast) {
    ...
    if (procedureNameToRelationshipMap.find(procedureCalledName) != procedureNameToRelationshipMap.
        end()) {
        vector<Relationship> procedureCalledRelationships = procedureNameToRelationshipMap.at(
            procedureCalledName);

        // procedure called relationships returns Modifies(p, v).
        // Reformat it to Modifies(call stmt, v)
        vector<Relationship> convertedRelationships = this->extractCallRelationshipFromProcedure(
            ast,
            procedureCalledRelationships);
        extractedRelationships.insert(
            extractedRelationships.end(),
            convertedRelationships.begin(),
            convertedRelationships.end());
...
vector<Relationship> ModifiesExtractor::recursiveContainerExtract(
    Entity& leftHandSide, shared_ptr<ASTNode> ast) {
    vector<Relationship> modifiesRelationships = vector<Relationship>();
    ASTNodeType type = ast->getType();
    ...
    case ASTNodeType::IF: {
        ...
        // Create relationship for parent of this container and the stmtlsts inside this if node
    ...
    case ASTNodeType::CALL: // helps handle indirect procedure call (e.g. A calls B which calls C)
    {
        ...
        shared_ptr<ProcedureASTNode> calledProcNode = dynamic_pointer_cast<ProcedureASTNode>(
            procedureToExtract);
