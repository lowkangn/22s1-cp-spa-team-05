...
private:
    /*
        ...
        1. We use this map to look ahead from the root program node and add the list of procedures,
        each mapping to its list of Uses(proc, v).

        2. For a call, use this map to get the variables used by the called procedure

        3. If called procedure has been extracted before, use the entries in this map (DP-style)

        4. Otherwise, the procedure will not be in this map. Add this procedure and its extracted
        relationships to the map for future use (i.e. in procedure calls)
    */
...
public:
    ...
    */
    explicit UsesExtractor(bool willCheckCallCycles = true)
    ...
    */
    ...
    /*
        This method forms uses relationships between the given LHS entity
        and all the given variables
    */
    void addRelationshipsWithVariables(
        vector<Relationship>& relationships, Entity& LHS, vector<Entity>& variables);
