...
private:
    /*
        This is used for performing DP operations on Modifies Call relationships.
        It maps the procedure name to relationships extracted.

        Parser guarantees no duplicate naming of procedures from the root program node,
        so mapping should be unique

        1. We use this map to look ahead from the root program node and add the list of
        procedures names, each mapping to a list of ModifiesRelationships extracted from it.

        2. For a call, we use this map to get the variables modified by the called procedure.

        3. If the called procedure has been extracted before, use the entries in this map to
        generate Modifies(call, v)

        4. Otherwise, the procedure name and its extracted relationships will not be in this map.
        We then add this procedure and its extracted relationships to the map for future use.
    */
    ...
    */
    ...
    /*
        ...
        Set to true by default, but can be set to false in the constructor
        to speed up testing if needed
    */
    ...
    */
    ...
    vector<Relationship> extractCallRelationshipFromProcedure(
        shared_ptr<ASTNode> callNode, vector<Relationship> relationships) {
        ...
        for (int i = 0; i < relationships.size(); i++) {
            ...
            Relationship toAdd = Relationship::createModifiesRelationship(
                leftHandSide,
                rightHandSide);
    ...
    /*
        ...
        Modified DFS - instead of starting from the root node, start from each proc node and
        populate the callGraph based on which procedure calls another procedure
    */
    void constructCallGraph(unordered_map<string, shared_ptr<ASTNode>> procedures) {
        ...
        for (unordered_map<string, shared_ptr<ASTNode>>::iterator it = procedures.begin();
    ...
    /*
        ...
        @param	callGraph - adjacency list with procedure nodes as the vertex,
                            call relationships as the edge
        @param	visited - unordered_map of ASTNode to bool indicating whether it is already visited
        @param	dfsVis - unordered_map of ASTNode to bool representing
                         which nodes are being processed in a single DFS call

        Idea: check if we already visited a node (which is also a neighbour of another node)
        If already visited, then we check whether we are processing its neighbours. If so,
        then we have found a cycle. Because we haven't finished processing the node's neighbours
        but have found an edge from the node to its neighbours.
    */
    bool checkCallCycleUtil(
        shared_ptr<ASTNode> currNode,
        unordered_map<shared_ptr<ASTNode>, vector<shared_ptr<ASTNode>>> callGraph,
        unordered_map<shared_ptr<ASTNode>, bool> visited,
        unordered_map<shared_ptr<ASTNode>, bool> dfsVis) {
        // Find currNode in vis and neighbourVis and set that node and
        // its neighbours to being visited (true)
        visited[currNode] = 1;
        dfsVis[currNode] = 1;
        ...
        for (shared_ptr<ASTNode> neighbour : callGraph.at(currNode)) {
            if (!visited.at(neighbour)) {
                ...
                if (checkCallCycleUtil(neighbour, callGraph, visited, dfsVis)) {
                    return true;
                }
            } else if (dfsVis.at(neighbour)) {
                // Node was visited before in same DFS call, there is a cycle
                return true;
            }
        }
    ...
    bool hasCallCycle(unordered_map<shared_ptr<ASTNode>, vector<shared_ptr<ASTNode>>> callGraph) {
        ...
        for (unordered_map<shared_ptr<ASTNode>, vector<shared_ptr<ASTNode>>>::iterator it =
                 callGraph.begin(); it != callGraph.end(); ++it) {
        ...
        for (unordered_map<shared_ptr<ASTNode>, bool>::iterator it = visited.begin();
            it != visited.end(); ++it) {
            // Check if curr vertex has been visited
            if (!it->second) {
                if (checkCallCycleUtil(it->first, callGraph, visited, dfsVis)) {
                    return true;
                }
            }
        }
...
public:
    ...
    */
    explicit ModifiesExtractor(bool willCheckCallCycles = true)
        : checkCallCycleFlag(willCheckCallCycles) {
    }
