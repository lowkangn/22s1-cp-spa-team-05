...
private:
    ...
    // Refer to https://stackoverflow.com/questions/68049611/no-default-constructor-exists-for-class-in-c  //NOLINT(whitespace/line_length)
    // for the following declaration usage
    Entity(EntityType type, int lineNumber, Token& token) : type(type), line(lineNumber),
                                                            token(token) {
        this->type = type;
...
public:
    ...
    */
    ...
    bool isOperator() {
        return this->token.isRelationalOperator()
            || this->token.isOperatorToken()
            || this->token.isNotOperator()
            || this->token.isConditionalOperator();
    }
