...
vector<shared_ptr<SimpleSyntaxRule>> WhileSimpleSyntaxRule::generateChildRules() {
    ...
    // Statement rule
    shared_ptr<SimpleSyntaxRule> stmtListRule = make_shared<StatementListSimpleSyntaxRule>();
...
list<Token> WhileSimpleSyntaxRule::consumeTokens(list<Token> tokens) {
    ...
    if (!token.isOpenCurlyBracketToken()) {
        ...
        throw SimpleSyntaxParserException(
            string(
                "WhileSyntaxRule: Expected first token to be open bracket, but was ") + token.
            getString());
    }
    ...
    this->tokens.insert(
        this->tokens.end(),
        this->statementListTokens.begin(),
        this->statementListTokens.end());
