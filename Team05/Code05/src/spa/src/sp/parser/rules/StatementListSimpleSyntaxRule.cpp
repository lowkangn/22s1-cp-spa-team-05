...
list<Token> StatementListSimpleSyntaxRule::consumeTokens(list<Token> tokens) {
    ...
    if (!token.isOpenCurlyBracketToken()) {
        throw SimpleSyntaxParserException(
            string(
                "StatementList: Expected first token to be open bracket, but was ") + token.
            getString());
    }
    tokens.pop_front();
...
vector<shared_ptr<SimpleSyntaxRule>> StatementListSimpleSyntaxRule::generateChildRules() {
    ...
    while (!tokens.empty()) {
        ...
        if (!(tokens.size() >= 2)) {
            throw SimpleSyntaxParserException(
                "Less than one token found while trying to parse for a statement in statement list"); //NOLINT(whitespace/line_length)
        } else if (this->isReadStatement(tokens)) {
            // is read statement
            shared_ptr<SimpleSyntaxRule> assignRulePointer = make_shared<ReadSimpleSyntaxRule>();
            tokens = assignRulePointer->consumeTokens(tokens);
            childRules.push_back(assignRulePointer); // add to children nodes in order
        } else if (this->isCallStatement(tokens)) {
            // call statement
            shared_ptr<SimpleSyntaxRule> callRulePointer = make_shared<CallSimpleSyntaxRule>();
            tokens = callRulePointer->consumeTokens(tokens);
            childRules.push_back(callRulePointer);
        } else if (this->isWhileStatement(tokens)) {
            // while block
            shared_ptr<SimpleSyntaxRule> whileRulePointer = make_shared<WhileSimpleSyntaxRule>();
        ...
        } else if (this->isIfStatement(tokens)) {
            // if else statement
            shared_ptr<SimpleSyntaxRule> ifRulePointer = make_shared<IfSimpleSyntaxRule>();
            ...
            shared_ptr<SimpleSyntaxRule> printRulePointer = make_shared<PrintSimpleSyntaxRule>();
            ...
            shared_ptr<SimpleSyntaxRule> readRulePointer = make_shared<AssignSimpleSyntaxRule>();
