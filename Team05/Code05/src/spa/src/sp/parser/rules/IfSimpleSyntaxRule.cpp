...
vector<shared_ptr<SimpleSyntaxRule>> IfSimpleSyntaxRule::generateChildRules() {
    ...
    // Conditional Rule
    shared_ptr<SimpleSyntaxRule> conditionRule = make_shared<
    ...
    // Then Statement rule
    shared_ptr<SimpleSyntaxRule> thenStmtListRule = make_shared<StatementListSimpleSyntaxRule>();
    tokens = thenStmtListRule->consumeTokens(this->thenTokens); // consume the tokens
    childRules.push_back(thenStmtListRule);

    // Else Statement rule
    shared_ptr<SimpleSyntaxRule> elseStmtListRule = make_shared<StatementListSimpleSyntaxRule>();
...
list<Token> IfSimpleSyntaxRule::consumeTokens(list<Token> tokens) {
    ...
    this->tokens.insert(
        this->tokens.end(),
        this->conditionTokens.begin(),
        this->conditionTokens.end());
...
list<Token> IfSimpleSyntaxRule::getStmtList(list<Token>& tokens) {
    ...
    if (!token.isOpenCurlyBracketToken()) {
        throw SimpleSyntaxParserException(
            string("Expected first token to be open curly bracket, but was ") + token.getString());
    }
